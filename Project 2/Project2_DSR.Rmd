...
# Original Dataset
koi_df$koi_disposition <- ifelse(koi_df1$koi_disposition == "FALSE POSITIVE", 0,
                     ifelse(koi_df1$koi_disposition == "CONFIRMED", 1, 2))

koi_df$koi_pdisposition[koi_df1$koi_pdisposition == "FALSE POSITIVE"] <- 0
koi_df$koi_pdisposition[koi_df1$koi_pdisposition == "CANDIDATE"] <- 1

koi_df$koi_disposition = as.factor(koi_df1$koi_disposition)
koi_df$koi_pdisposition = as.factor(koi_df1$koi_pdisposition)
...
koi_train_dis_orig <- koi_df1[koi_sample==1, -2]     # X and Y=koi_disposition
koi_test_dis_orig <- koi_df1[koi_sample==2, -2]      # X and Y=koi_disposition
koi_train_pdis_orig <- koi_df1[koi_sample==1, -1]    # X and Y=koi_pdisposition
koi_test_pdis_orig <- koi_df1[koi_sample==2, -1]     # X and Y=koi_pdisposition
koi_trainX_orig <- koi_df1[koi_sample==1, 3:23]      # Only X 
koi_testX_orig <- koi_df1[koi_sample==2, 3:23]       # Only X
koi_trainLabels_dis_orig <- koi_df1[koi_sample==1, 1]  # Only Y= koi_disposition
koi_testLabels_dis_orig <- koi_df1[koi_sample==2, 1]   # Only Y= koi_disposition
koi_trainLabels_pdis_orig <- koi_df1[koi_sample==1, 2] # Only Y= koi_pdisposition
koi_testLabels_pdis_orig <- koi_df1[koi_sample==2, 2]  # Only Y= koi_pdisposition

```

## Stepwise Feature Selection - I think this was a bad idea
...
loadPkg('caret')
# Fit Model
logit_mod_step = multinom(koi_disposition~., data = koi_train_dis_orig, model = T) %>%
  stepAIC(trace = FALSE)
# summary(logit_mod_step)
# Predictions
logit_mod_step_prob = logit_mod_step %>% predict(koi_test_dis_orig, type = 'response')
logit_mod_predicted.classes = ifelse(logit_mod_step_prob = 1, "CANDIDATE", "FALSE POSITIVE")
# Model Acccuracy
mean(logit_mod_predicted.classes==koi_test_dis_orig$koi_disposition)
'
```
