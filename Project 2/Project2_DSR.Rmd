...
Since a random forest requires a binary target, we decided to adjust our target feature to only contain two options - 1 for confirmed and 0 for false positive or candidate since a candidate is not a confirmed exoplanet.
...
```{r step-wise feature selections, eval=FALSE}
# Fit Model
...
```
''''
## 3.3 Variable Importance Test  

```{r, fig.width = 14, fig.length=12}
# set.seed(1000)
# boruta.model = Boruta(koi_disposition~., data=koi_df[-2], doTrace = 2, maxRuns = 50)
# boruta.model
# plot(boruta.model, las = 2 , cex.axis = 0.7)


## This takes too long. But it seems all the variables from the original dataset are important                      
```
