...
loadPkg("psych")
corr_mat <- corr.test(subset(citydf2,select = -c(region_name,state_name)))
corrplot::corrplot(cor(subset(citydf2,select = -c(region_name,state_name))),method = 'number',type = "lower",tl.pos = "lt")
corrplot::corrplot(cor(subset(citydf2,select = -c(region_name,state_name))),method = 'circle',type = "upper",tl.pos = "n",sig.level = 0.05,p.mat = corr_mat$p,add = T)
title(main = "Corrplot")
summary(citydf2)
citydf2$region_name <- factor(citydf2$region_name)
citydf2$state_name <- factor(citydf2$state_name)
```
